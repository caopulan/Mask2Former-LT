...
def convert(input, output):
    ...
    new_data = np.ones(img.shape, dtype=np.uint8) * 255
    for k in uni:
        ori_label, new_label = k, stuff_dataset_id_to_contiguous_id[k]
        pos = img == ori_label
        new_data[pos] = new_label
    Image.fromarray(new_data).save(output)


if __name__ == "__main__":
    dataset_dir = Path(os.getenv("DETECTRON2_DATASETS", "datasets")) / "coco_stuff_118k"
    for name in ["train2017", "val2017"]:
        annotation_dir = dataset_dir / "stuffthingmaps_trainval2017" / name
        output_dir = dataset_dir / "annotations" / name
