...
class SeesawLoss(nn.Module):
    ...
    def forward(self,
        ...
        #     sample_num = inds_.sum()
        #     dist.all_reduce(sample_num)
        #     self.cum_samples[u_l] += sample_num
        for u_l in range(self.num_classes):
            inds_ = labels == u_l
            sample_num = inds_.sum()
            dist.all_reduce(sample_num)
            self.cum_samples[u_l] += sample_num
        ...
        if objectness_weight is None:
            objectness_weight = 1.
        loss_cls = loss_cls_classes + objectness_weight * loss_cls_objectness
        return loss_cls