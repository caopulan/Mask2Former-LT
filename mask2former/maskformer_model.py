...
class MaskFormer(nn.Module):
    ...
    def __init__(
        ...
        if size_divisibility < 0:
            ...
            size_divisibility = self.backbone.size_divisibility  # 32
    ...
    def from_config(cls, cfg):
        backbone = build_backbone(cfg)  # cfg.MODEL.BACKBONE.NAME = "build_resnet_backbone" or "D2SwinTransformer"
        sem_seg_head = build_sem_seg_head(cfg, backbone.output_shape())  # MSDeformAttnPixelDecoder
        ...
        matcher = HungarianMatcher(
            ...
            loss_type=loss_type,
            match_method=cfg.MATCHER.MODE,
            dataset_name=cfg.MATCHER.DATASET,
            t=cfg.MATCHER.T,
            s=cfg.MATCHER.S,
        )

        weight_dict = {"loss_ce": class_weight, "loss_seesaw": class_weight, "loss_mask": mask_weight, "loss_dice": dice_weight}

        if deep_supervision:
            dec_layers = cfg.MODEL.MASK_FORMER.DEC_LAYERS  # 10 = 9 decoder layers, one for the loss on learnable query
            aux_weight_dict = {}
        ...
        criterion = SetCriterion(
            ...
            loss_type=loss_type,
            seesaw_q=cfg.LOSS.SEESAW_Q,
            seesaw_p=cfg.LOSS.SEESAW_P,
        )
        ...
        return {
            ...
            "sem_seg_postprocess_before_inference": (
                    cfg.MODEL.MASK_FORMER.TEST.SEM_SEG_POSTPROCESSING_BEFORE_INFERENCE  # False, semseg post off
    ...
    def semantic_inference(self, mask_cls, mask_pred):
        if self.loss_type == 'Seesaw':
            objectness = F.softmax(mask_cls[:, -2:])
            mask_cls = F.softmax(mask_cls[:, :-2], dim=-1) * objectness[:, :1]
        else:
